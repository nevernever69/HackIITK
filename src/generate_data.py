...
def load_raw_logs(log_folder):
    ...
    """
    log_files = [os.path.join(log_folder, file) for file in os.listdir(log_folder) if file.endswith('.csv')]
    
    # Check if any CSV files were found
    if not log_files:
        print(f"Warning: No CSV files found in the folder: {os.path.abspath(log_folder)}")
        return pd.DataFrame()
...
def generate_features(preprocessed_logs):
    ...
    if 'activity' in preprocessed_logs.columns:
        features['logon_count'] = preprocessed_logs[preprocessed_logs['activity'] == 'Logon'].groupby(['user', 'pc']).size().reset_index(name='logon_count')
        features['after_hours'] = preprocessed_logs[(preprocessed_logs['hour'] < 8) | (preprocessed_logs['hour'] > 18)].groupby(['user', 'pc']).size().reset_index(name='after_hours')
        features['thumb_drive_usage'] = preprocessed_logs[preprocessed_logs['activity'] == 'connect'].groupby(['user', 'pc']).size().reset_index(name='thumb_drive_usage')

    if 'url' in preprocessed_logs.columns:
        features['url_count'] = preprocessed_logs.groupby(['user', 'pc'])['url'].count().reset_index(name='url_count')

    # Merge all features into a single DataFrame
    structured_data = features['logon_count']
    for key, df in features.items():
        if key != 'logon_count':
            structured_data = pd.merge(structured_data, df, on=['user', 'pc'], how='left')
